import { useGetTasksQuery, useUpdateTaskStatusMutation, TaskType } from '@/state/api';
...
const Boardview = ({ id, setIsModelNewTaskOpen }: BoardProps) => {
    ...
    // Grabbing the data with project id 
    const { data: tasks, isLoading, error } = useGetTasksQuery({ projectId: Number(id) })

    // Moving the task
    // this is the trigger function that will call the api 
            ...
            <div className='mb-3 flex w-full'>
                ...
                <div className='flex w-full items-center justify-between rounded-e-lg bg-white px-5 py-4 dark:bg-dark-secondary'>
                    ...
                    <div className='flex items-center gap-1'>
                        <button className='flex h-6 w-5 items-center justify-center dark:text-netural-500 '>
...
const Task = ({ task }: TaskProps) => {
    ...
    const formatttedStartDate = task.startDate ? format(new Date(task.startDate), "P") : "";

    const formatttedDueDate = task.dueDate ? format(new Date(task.dueDate), "P") : "";
